...
<ul class="nested-comments">
	{#each comments as comment}
		<li
			class="comment-item"
			style="border-left: 4px solid {colors[
				level % colors.length
		...
		>
			...
			<div class="mb-2 flex items-center gap-3">
				...
				<button class="dislike-btn" aria-label="Dislike"
					><span>👎</span> {comment.dislikes ?? 0}</button
				>
				<button class="reply-btn" on:click={() => toggleReply(comment.id)} aria-label="Reply"
					>Reply</button
				>
			</div>
			{#if replying[comment.id]}
				<div class="mb-2 pr-2">
					<textarea
						class="min-h-[40px] w-full rounded border px-2 py-1"
						placeholder="Write your reply..."
					></textarea>
					<div class="mt-1 flex gap-2">
						<button class="send-btn" disabled>Send</button>
						<button class="cancel-btn" type="button" on:click={() => toggleReply(comment.id)}
							>Cancel</button
						>
					</div>
				</div>
