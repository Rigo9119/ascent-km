<script lang="ts">
	...
	const { cardItem, urlSection, user }: { cardItem: Item; urlSection: string; user?: User } =
		$props();
...
<Card.Root class="flex h-full flex-col py-0">
	<Card.Header class="relative p-0">
		...
		{#if user}
			<Button
				variant="ghost"
				size="icon"
				class="absolute top-2 right-2 cursor-pointer rounded-full bg-white/80 backdrop-blur-sm hover:bg-white"
	...
	<Card.Footer class="p-4 pt-0">
		<Button
			class="w-full cursor-pointer bg-emerald-500 hover:bg-emerald-600"
			onclick={() => viewDetails(cardItem.id)}>More</Button
		>
	</Card.Footer>
</Card.Root>
