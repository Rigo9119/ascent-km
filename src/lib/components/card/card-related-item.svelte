  ...
  <div class="relative aspect-[4/3] overflow-hidden">
    ...
    {#if 'category' in relatedItem && relatedItem.category}
      <div class="absolute top-2 left-2">
        <span
          class="rounded-full px-2 py-1 text-xs font-medium {getCategoryColor(relatedItem.category)}"
        >
    ...
    {#if 'date' in relatedItem && relatedItem.date}
      <div class="text-muted-foreground flex items-center justify-between text-xs">
        ...
        <div class="flex items-center gap-1">
          ...
          <span class="line-clamp-1">{'location' in relatedItem ? relatedItem.location : 'N/A'}</span>
        </div>
      </div>
    {/if}
