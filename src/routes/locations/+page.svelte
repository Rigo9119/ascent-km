<script lang="ts">
	...
	const { data }: { data: { locations: AppLocation[] } } = $props();
...
<div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 sm:py-6 md:px-8 md:py-8">
	...
	<section class="mb-6 sm:mb-8">
		<div class="h-64 overflow-hidden rounded-lg sm:h-80 md:h-96 lg:h-[500px]">
			{#if !apiKey}
				<div class="bg-muted flex h-full items-center justify-center rounded-lg">
					<p class="px-4 text-center text-sm sm:text-base md:text-lg">
	...
	<section>
		<h2 class="mb-4 text-xl font-semibold sm:mb-6 sm:text-2xl md:text-3xl">Popular Locations</h2>
		<ul
